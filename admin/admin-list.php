            ...
            <div class="ak-card-body">
                <form method='post' class='ak-admin-form'>
                    ...
                    <div class="ak-admin-grid">
                        <div style="grid-column: span 2;"><label>Név</label><input name='name'
                                value='<?php echo esc_attr($r->name); ?>'></div>
                    ...
                    <div class="ak-admin-grid">
                        <div><label>Érkezés dátuma</label><input type='date' name='arrival' value='<?php echo $r->arrival; ?>'>
                        </div>
                        <div>
                            <label>Csomag</label>
                            <label>Csomag</label>
                            <select name="package">
                                ...
                                <?php foreach (ak_get_active_packages() as $pkg): ?>
                                    <option value="<?php echo esc_attr($pkg); ?>" <?php selected($r->package, $pkg); ?>>
                                        <?php echo esc_html($pkg); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="status-wrapper">
                                <select class="ak-status-select status-<?php echo $r->status; ?>"
                                    data-id="<?php echo $r->id; ?>">
                                    <option value="uj" <?php selected($r->status, 'uj'); ?>>Új</option>
                                    <option value="feldolgozva" <?php selected($r->status, 'feldolgozva'); ?>>Feldolgozva
                                    </option>
                                </select>
                                <span class="save-indicator">✓</span>
                            </div>
                        </td>
                        <td><a class='button button-small' href='?page=ajanlatok&edit=<?php echo $r->id; ?>'>Szerkesztés</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
